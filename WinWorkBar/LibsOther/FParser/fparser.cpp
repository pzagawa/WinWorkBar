                                 ...
                                 FunctionPtr func, unsigned paramsAmount)
{
    ...
    if(isValidName(name))
    {
        ...
        data->FuncPtrNames[name] = (unsigned)data->FuncPtrs.size();
                                 ...
                                 FunctionParser& parser)
{
    ...
    if(isValidName(name))
    {
        ...
        data->FuncParserNames[name] = (unsigned)data->FuncParsers.size();
                          ...
                          bool useDegrees)
{
    ...
    if(!Compile(Func)) return (int)Function.size();
...
int FunctionParser::CompileIf(const char* F, int ind)
{
    ...
    AddCompiledByte(cIf);
    unsigned curByteCodeSize = (unsigned)tempByteCode->size();
    ...
    unsigned curByteCodeSize2 = (unsigned)tempByteCode->size();
    unsigned curImmedSize2 = (unsigned)tempImmed->size();
    AddCompiledByte(0); // Jump index; to be set later
    AddCompiledByte(0); // Immed jump index; to be set later
    ...
    (*tempByteCode)[curByteCodeSize2] = (unsigned)tempByteCode->size()-1;
    (*tempByteCode)[curByteCodeSize2+1] = (unsigned)tempImmed->size();
...
int FunctionParser::CompileElement(const char* F, int ind)
{
    ...
    if(isdigit(c) || c=='.' /*|| c=='-'*/) // Number
    {
        ...
        incStackPtr();
        return (int)ind + (int)(endPtr-startPtr);
    }
    ...
    if(isalpha(c) || c == '_') // Function, variable or constant
    {
        ...
        if(func) // is function
        {
            ...
            unsigned requiredParams =
                ...
                (unsigned)data->Variables.size() : func->params;
#else
        ...
        if(vIter != data->Variables.end()) // is variable
        {
            ...
            incStackPtr();
            return (int)ind + (int)vIter->first.size();
        }
        ...
        if(cIter != data->Constants.end()) // is constant
        {
            ...
            AddCompiledByte(cImmed);
            incStackPtr();
            return (int)ind + (int)cIter->first.size();
        }
        ...
        if(fIter != data->FuncPtrNames.end()) // is user-defined func pointer
        {
            ...
            int ind2 = (int)ind + (int)fIter->first.length();
            sws(F, ind2); // F[ind2] is '('
        ...
        if(pIter != data->FuncParserNames.end()) // is user-defined func parser
        {
            ...
            int ind2 = (int)ind + (int)pIter->first.length();
            sws(F, ind2); // F[ind2] is '('
...
public:
    ...
    unsigned GetArgCount() const { return (unsigned)data->args.size(); }
...
private:
    struct ConstList
    {
        ...
        list<pit> cp;
        double value;
        unsigned size() const { return (unsigned)cp.size(); }
    };
    ...
    vector<double>   &immed) const
{
    ...
    switch(GetOp())
    {
        ...
        case cIf:
        {
            ...
            unsigned ofs = (unsigned)byteCode.size();
            AddCmd(cIf);
            AddCmd(0); // code index
