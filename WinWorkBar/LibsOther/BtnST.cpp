...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		...
		if (m_csIconDisabled.hIcon)	::DestroyIcon(m_csIconDisabled.hIcon);
	...
	::ZeroMemory(&m_csIconDisabled, sizeof(m_csIconDisabled));
...
void CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (bIsDisabled)
	{
		PrepareImageRect(bHasTitle, rpItem, rpCaption, bIsPressed, m_csIconDisabled.dwWidth, m_csIconDisabled.dwHeight, &rImage);
		pDC->DrawState(rImage.TopLeft(), rImage.Size(), m_csIconDisabled.hIcon, DSS_NORMAL, (CBrush*)NULL);
	} else {
		PrepareImageRect(bHasTitle, rpItem, rpCaption, bIsPressed, m_csIcons[byIndex].dwWidth, m_csIcons[byIndex].dwHeight, &rImage);
		pDC->DrawState(rImage.TopLeft(), rImage.Size(), m_csIcons[byIndex].hIcon, DSS_NORMAL, (CBrush*)NULL);
	}
	//was: (bIsDisabled ? DSS_DISABLED : DSS_NORMAL),
} // End of DrawTheIcon
...
DWORD	SetDisabledIcon(bool GenerateGrayScale = true, int nIconIn = 0)
{
	//if (GenerateGrayScale)
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	if (hIconIn)
	{
		...
		m_csIconDisabled.hIcon = CreateGrayscaleIcon(hIconIn);;
		// Get icon dimension
		::ZeroMemory(&ii, sizeof(ICONINFO));
		bRetValue = ::GetIconInfo(m_csIconDisabled.hIcon, &ii);
		if (bRetValue == FALSE)
		{
			FreeResources();
			return BTNST_INVALIDRESOURCE;
		} // if
		m_csIconDisabled.dwWidth	= (DWORD)(ii.xHotspot * 2);
		m_csIconDisabled.dwHeight	= (DWORD)(ii.yHotspot * 2);
		::DeleteObject(ii.hbmMask);
		::DeleteObject(ii.hbmColor);
...
void	CButtonST::SetMenuItemText(UINT ID, LPCTSTR S)
{
	ModifyMenu(m_hMenu, ID, MF_BYCOMMAND, ID, S);
}

void	CButtonST::SetMenuItemEnable(UINT ID, bool Enabled)
{
	ModifyMenu(m_hMenu, ID, MF_BYCOMMAND | ((Enabled)?MF_ENABLED:MF_GRAYED), ID, NULL);
}
